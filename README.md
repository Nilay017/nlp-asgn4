...
```
**_Note_** - The code may complain file not found error. That's because the testing script is looking for a file with specific name. In that case, just rename our model file present in ```our_models/``` directory to whatever the testing script requires. The models are large and uploaded with git lfs. Sometimes name of file get pruned with git lfs.
