...
from evaluation.compute_bleu import out_beam_act
...
def output_beam(translation_corpus, dataset_name, split, tokenizer=None, section=None, args=None):
    dataset = os.path.join('data/{}/{}.json'.format(dataset_name, split))
    with open(dataset) as dataset_file:
        dataset_object = json.loads(dataset_file.read())
    with open('data/{}/{}_order.json'.format(dataset_name, split), 'rb') as f:
    ...
    for index in section:
        ...
        #         translation[i] = translation[i]['str']            
        #     translation_corpus[index] = translation
    ...
    out_beam_act(translation_corpus, dataset_object, section, args=args)
